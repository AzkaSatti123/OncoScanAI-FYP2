% =============================================================================
% Overleaf-ready Chapter 3 — paste as main.tex OR merge into your project
...
\usepackage{parskip}
\usepackage{xcolor}
% verbatim blocks for Mermaid (avoids listings + special chars issues)
\usepackage{verbatim}

\hypersetup{colorlinks=true,linkcolor=blue,urlcolor=blue}

% Slightly tighter tables; reduce overfull boxes
...
Figure~\ref{fig:box-line} shows high-level connectivity. Paste the code below into \texttt{mermaid.live} to export a diagram image if required.

\begin{figure}[htbp]
  \centering
  {\small\textit{Mermaid source (box-and-line):}}
  \vspace{0.5em}
  \begin{verbatim}
flowchart LR
  ...
  API --> FS
  \end{verbatim}
  \caption{Box-and-line diagram (Mermaid source)}
  \label{fig:box-line}
\end{figure}
...
\begin{table}[htbp]
  ...
  \begin{tabular}{@{}p{0.16\textwidth}p{0.30\textwidth}p{0.48\textwidth}@{}}
    \toprule
...
flowchart TB
  ...
  F --> M
  \end{verbatim}
  \caption{Major subsystems and connections (Mermaid source)}
\end{figure}
...
erDiagram
  ...
  \end{verbatim}
  \caption{Logical ERD (Mermaid source)}
\end{figure}
...
flowchart LR
  ...
  \end{verbatim}
  \caption{DFD Level 0 (context; Mermaid source)}
\end{figure}
...
flowchart TB
  ...
  P5 --> E
  \end{verbatim}
  \caption{DFD Level 1 (Mermaid source)}
\end{figure}
...
classDiagram
  ...
  \end{verbatim}
  \caption{Domain-oriented class diagram (Mermaid source)}
\end{figure}
...
flowchart TD
  ...
  \end{verbatim}
  \caption{Activity diagram: histology pipeline (Mermaid source)}
\end{figure}
...
sequenceDiagram
  ...
  \end{verbatim}
  \caption{Sequence diagram: class-level interaction (Mermaid source)}
\end{figure}
...
stateDiagram-v2
  ...
  \end{verbatim}
  \caption{State diagram for upload/analysis lifecycle (Mermaid source)}
\end{figure}
...
flowchart LR
  ...
  \end{verbatim}
  \caption{Component diagram (Mermaid source)}
\end{figure}
